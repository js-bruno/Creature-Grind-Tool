...
class BattleMode:
    ...
    def run(self):
        in_battle = self._autogui.locateOnScreen(
            image=self._image_storage.hp_label_battle_image(), grayscale=True
        )
    ...
    def _attack_with_efficient_move(self):
        super_effetive_move = self._autogui.locateOnScreen(
            self._image_storage.load_super_effetive_image()
        )
